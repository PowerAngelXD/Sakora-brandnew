...
namespace sakora {
    ...
    public:
        /* 构造函数，用于构造一个sakVM指令实例
           ...
           args     意指参数
        */
        VMCode(InsOp op, int ln, int col, std::initializer_list<sakora::Value> _args = {});

        const InsOp& getOp();
        std::vector<sakora::Value> getArgs();
        sakora::Value getArg(int pos = 0);
